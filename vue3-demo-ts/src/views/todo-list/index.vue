...
// computed
const filterTodoList = computed<Array<Todo>>(() => {
  let list = todoList
...
const addItem = (value:string) => {
  const id = `item${todoList.length}`
  todoList.push({ id, title: value, checked: false })
}
// 删除
...
// 修改
const changeCheck = (todoItem:Todo) => {
...
// 过滤
const filterTodo = ({ value }:{value:string}) => {
